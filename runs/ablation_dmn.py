...
print("RESULT use_dmn={} ppl={:.2f} loss={:.4f} sw={} plain={:.1f}% mode={}".format(
    USE_DMN, vppl, vloss, sw, plain_pct, mode))
'''

results = []
for use_dmn in (True, False):
    script = INNER.strip().replace("{ROOT}", str(ROOT)).replace("{USE_DMN}", str(use_dmn))
    out = subprocess.run([sys.executable, "-c", script], cwd=str(ROOT), capture_output=True, text=True)
    print("=== use_dmn={} ===".format(use_dmn), flush=True)
    if out.returncode != 0:
        print("ERROR:", out.stderr[-1500:], flush=True)
    else:
        for line in out.stdout.strip().splitlines():
            if "RESULT" in line or "use_dmn" in line or " ppl=" in line:
                print(line, flush=True)
                results.append(line)
    print(flush=True)

print("\nSUMMARY TABLE")
print("use_dmn    val_ppl  val_loss  switches  plain%   mode")
print("-" * 48)
for line in results:
    import re
    m = re.search(r"use_dmn=(\S+).*?ppl=([\d.]+).*?loss=([\d.]+).*?sw=(\d+).*?plain=([\d.]+)%.*?mode=(\S+)", line)
    if m:
        print("{:<10} {:>8} {:>9} {:>8} {:>8}% {:>10}".format(
