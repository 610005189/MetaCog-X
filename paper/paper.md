...
实验结果表明，MetaCog-X在保持元认知开销平均低于5%的同时，达到了与基线模型相当或更好的性能。条件激活机制成功减少了不必要的计算，同时保留了在具有挑战性的推理步骤中元认知处理的优势。
...
在元模式下，注意力通过元信号和觉知信号增强：

```
Attention = ContentAttention + MetaAttention + AwarenessAttention
```

这种三重注意力机制允许模型将元认知信息整合到推理过程中。
...
| 模型 | PPL | Loss | 开销 |
|------|-----|------|------|
| GPT Plain | 1.28 | 0.2436 | 0% |
| MetaCog Always-On | 1.34 | 0.2910 | 4.7% |
| MetaCog Conditional | 1.34 | 0.2910 | 4.7% |
...
| 指标 | 值 | 阈值 | 状态 |
|------|-----|------|------|
| ctrl_std | 0.0548 | > 0.05 | ✅ |

### 5.5 分析

我们的结果表明：

1. **条件激活有效**：L1门控成功检测认知困境
2. **开销可控**：元认知计算仅在约11%的时间内应用
